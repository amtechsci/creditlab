<div class="all-content-wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="logo-pro">
                        <!--<a href="index.html"><img class="main-logo" src="img/logo/logo.png" alt="" /></a>--><h1>Creditlab</h1>
                    </div>
                </div>
            </div>
        </div>
        <div class="header-advance-area">
            <div class="header-top-area">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <div class="header-top-wraper">
                                <div class="row header-top-inner">
                                    <div class="col-lg-1 col-md-0 hidden-xs hidden-sm">
                                        <div class="menu-switcher-pro">
                                            <button type="button" id="sidebarCollapse" class="btn bar-button-pro header-drl-controller-btn btn-info navbar-btn">
                                                    <i class="educate-icon educate-nav"></i>
                                                </button>
                                        </div>
                                    </div>
                                    <div class="col-lg-6 col-md-7 hidden-xs hidden-sm">
                                        <div class="header-top-menu tabl-d-n">
                                            <form action="sea.php" method="get" target="_blank" style="margin-top:10px; display:flex; gap:8px; align-items:center;">
                                                <input type="text" name="sea" class="form-control" placeholder="Search users..." style="height:34px; max-width:280px; border-radius:4px;">
                                                <button type="submit" class="btn btn-success" style="height:34px; padding:6px 14px;">Search</button>
                                            </form>
                                           
                                        </div>
                                    </div>
                                    <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12 header-right-col">
                                        <div class="header-right-info">
                                            <a href="<?=getAppUrl()?>/" class="btn btn-success header-view-site" target="_blank">view site</a>
                                            <ul class="nav navbar-nav mai-top-nav header-right-menu">
                                               
                                                <li class="nav-item">
                                                     
                                                    <a href="#" data-toggle="dropdown" role="button" aria-expanded="false" class="nav-link dropdown-toggle">
                                                        
                                                            <i class="fa fa-user-circle" style="font-size:28px;vertical-align:middle;" aria-hidden="true"></i>
                                                            <span class="admin-name"><?=$user_name;?></span>
                                                            <i class="fa fa-angle-down edu-icon edu-down-arrow"></i>
                                                        </a>
                                                        
                                                    <ul role="menu" class="dropdown-header-top author-log dropdown-menu animated zoomIn">
                                                        <!--<li><a href="changepassword.php"><span class="edu-icon edu-locked author-log-ic"></span>Change password</a>
                                                        </li>-->
                                                        <li><a href="logout.php"><span class="edu-icon edu-locked author-log-ic"></span>Log Out</a>
                                                        </li>
                                                    </ul>
                                                </li>
                                                
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                                <form class="header-mobile-search" action="sea.php" method="get" target="_blank">
                                    <input type="text" name="sea" placeholder="Search users, loans, mobile..." autocomplete="off">
                                    <button type="submit" aria-label="Search"><i class="fas fa-search"></i></button>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
            </div>