...
// if(isset($_POST['sms_num']) and isset($_POST['sms'])){
//     $message = $_POST['sms'];
// $url = "https://digitalsupporter.in/api/send.php"; 
// $ch = curl_init();  
// curl_setopt($ch, CURLOPT_URL,$url);
// curl_setopt($ch, CURLOPT_POST, 1);
// curl_setopt($ch, CURLOPT_POSTFIELDS,
//             "senderid=RSALRY&message=".$message."&number=".$_POST['sms_num']."&apikey=rush4cash@sms");
  ...
  <center><p id="check"></p>
    ...
    <input type="submit" name="reff" class="btn btn-sucess">
        ...
        <tbody>
                                   ...
                                   <?php $loan_data = towquery("SELECT * FROM loan WHERE uid='$userpro_id' ORDER BY id DESC"); while($loan_fetch = towfetch($loan_data)){ extract($loan_fetch,EXTR_PREFIX_ALL,"users");

        $loan_dataa = towquery("SELECT * FROM loan WHERE lid='$users_lid' ORDER BY id DESC"); $loan_fetcha = towfetch($loan_dataa); extract($loan_fetcha,EXTR_PREFIX_ALL,"usersd");
                                   ?>
                                    <form action="" method="post"><tr>
                                        ...
                                        <td><?=$usersd_p_fee?></td>
                                        <?php if($users_is_emi==0){?><td><?=$usersd_service_charge;?></td>
                                        <td><?=$usersd_penality_charge?></td>
                                        <td><?=date('Y-m-d', strtotime($usersd_processed_date . ' +30 day'))?></td>
                                        <td><?=$usersd_total_amount?></td><?php }?>  
                                        <?php if($users_is_emi==1){
                                        $a = towquery("SELECT * FROM `loan_apply` WHERE id=$usersd_lid");
             $b = towfetch($a);
             ...
             $day_gap = round($datediff / (60 * 60 * 24));
                 ...
                 $semi_amount = $loan_amountc / 2 + $loan_amountc * 0.005 * $seday_gap + $tax / 2;
                                        ...
                                        <td>Rs. <?=$femi_amount?> <?php if($lof['femi']){echo '✔️';}?> <br> Rs. <?=$semi_amount?> <?php if($lof['semi']){echo '✔️';}?></td>
                                        <?php }else{ ?>
                                        <td></td>
                                        <td></td>
                                        <?php }?>
                                        <td><?=$usersd_status_log?></td>
                                        <td><?php if($usersd_action == "no data"){?><a class="btn btn-success" href="updateloan.php?id=<?=$usersd_id?>&status=cleared">Cleared</a> <a class="btn btn-danger" href="updateloan.php?id=<?=$usersd_id?>&status=default">Default</a><?php }else{echo $usersd_action;}?></td>
                                    </tr></form>
                                <?php } ?>
            </tbody>
    </table>
