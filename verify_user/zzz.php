...
?>
                                  ...
                                  echo 'user'.$users_id.' '.$value.'<br>';
                                   $i++;
                                   }
                                   $ref = towquery("SELECT * FROM `user_ref` WHERE (`ref_1` LIKE '%$value%' OR `ref_2` LIKE '%$value%' OR `ref_3` LIKE '%$value%' OR `ref_4` LIKE '%$value%' OR `ref_5` LIKE '%$value%') AND NOT uid=$id");
                                   while($reffetch = towfetch($ref)){ extract($reffetch,EXTR_PREFIX_ALL,"users");
                                   $seauserid[$i] = $users_uid;
                                  echo 'user ref'.$users_uid.' '.$value.'<br>';
                                   $i++;
                                   }
                                   $usercontact = towquery("SELECT * FROM `user_contact_details` WHERE (`user_contact` LIKE '%$value%') AND NOT uid=$id");
                                   while($usercontactfetch = towfetch($usercontact)){ extract($usercontactfetch,EXTR_PREFIX_ALL,"users");
                                   $seauserid[$i] = $users_uid;
                                  echo 'contact'.$users_uid.' '.$value.'<br>';
                                   $i++;
                                   }}
                                   }
                                   ?>
