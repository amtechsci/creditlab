    ...
    */
    ...
    class Easebuzz{
        ...
        */
        ...
        public function initiateAutoDebitAuthorization($params) {
        // Endpoint URL for Auto Debit Authorization
        $url = $this->ENV === 'test' ? 'https://testpay.easebuzz.in/payment/initiateAutoDebitAuthorization' : 'https://pay.easebuzz.in/payment/initiateAutoDebitAuthorization';
        
        // Generate the hash
        $params['hash'] = $this->_getHashKey($params);
        
        // Send the request
        $response = $this->_curlCall($url, $params);

        return json_decode($response, true);
    }

    /**
     * Initiates Auto Debit Request
     *
     * @param array $params - holds the required data for auto debit request
     * @return array $response - API response from Easebuzz
     */
    public function initiateAutoDebitRequest($params) {
        // Endpoint URL for Auto Debit Request
        $url = $this->ENV === 'test' ? 'https://testpay.easebuzz.in/payment/initiateAutoDebitRequest' : 'https://pay.easebuzz.in/payment/initiateAutoDebitRequest';
    ...
    /**
     * Generate the hash key for the API request
     *
     * @param array $data - Data array for which the hash is to be generated
     * @return string $hash - Generated hash key
     */
    private function _getHashKey($data) {
     ...
     * @param array $params - POST data to be sent
     * @return string $response - cURL response from the API
     */
    private function _curlCall($url, $params) {
        $ch = curl_init($url);
        ...
        if (curl_errno($ch)) {
            ...
            curl_close($ch);
            return json_encode(['status' => 0, 'error_desc' => 'cURL error: ' . $error_msg]);
        }
