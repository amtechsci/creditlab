...
if (!$result['ok']) {
    ...
    $log_file = isset($result['log_file']) ? basename($result['log_file']) : basename(creditlab_easebuzz_enach_log_path());
    $txnid = isset($result['txnid']) ? $result['txnid'] : '';
    ...
    if ($txnid !== '') {
        echo '<p><strong>Reference txnid:</strong> ' . htmlspecialchars($txnid, ENT_QUOTES) . '</p>';
    }
